...
use Webkul\Fawry\Exceptions\MissingPaymentInfoException;
use Webkul\Fawry\Interfaces\PaymentInterface;
use Webkul\Fawry\Classes\BaseController;
...
class FawryPayment extends BaseController implements PaymentInterface
{
    ...
    {
        $this->fawry_url = config('payments.FAWRY_URL');
        $this->fawry_merchant = config('payments.FAWRY_MERCHANT');
        $this->fawry_secret = config('payments.FAWRY_SECRET');
        $this->fawry_display_mode = config('payments.FAWRY_DISPLAY_MODE');
        $this->fawry_pay_mode = config('payments.FAWRY_PAY_MODE');
        $this->verify_route_name = config('payments.VERIFY_ROUTE_NAME');
    }
    ...
    public function verify(Request $request)
    {
        ...
        if ($response->offsetGet('statusCode') == 200 && $response->offsetGet('paymentStatus') == "PAID") {
            return [
                ...
                'message' => __('fawry::messages.PAYMENT_DONE'),
    ...
    {
        return view('fawry::admin.index', ['model' => $this, 'data' => $data])->render();
    }
